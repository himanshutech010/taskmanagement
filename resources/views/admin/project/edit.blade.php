            ...
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="forms-sample" method="POST" action="{{ route('admin.project.update', $project->id) }}"
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select id="department" name="department_id" class="form-control form-control-sm"
                                        required onchange="loadEmployees(this.value)">
                                        <option value="" disabled>Choose...</option>
                                                ...
                                                @if ($emp->status == 0)
                                                    <div class="form-check">
                                                        <input type="checkbox" class="form-check-input"
                                                            id="employee-{{ $employee }}" name="employees[]"
                                                            value="{{ $employee }}"
                                                            onchange="updateModeratorDropdown()"
                                                            {{ in_array($employee, old('employees', $assignedEmployees)) ? 'checked' : '' }}>
                                                        <label class="form-check-label"
                                                            for="employee-{{ $employee }}"><span
                                                                class="text-danger">{{ $emp->name }}(Inactive)</span></label>
                                                    </div>
                                                @else
                                                    <div class="form-check">
                                                        <input type="checkbox" class="form-check-input"
                                                            ...
                                                            {{ in_array($employee, old('employees', $assignedEmployees)) ? 'checked' : '' }}>
                                                        <label class="form-check-label"
                                                            for="employee-{{ $employee }}">{{ $emp->name }}</label>
                                                    </div>
                                                @endif
                                            @endif
                                        @endforeach
                            ...
                            <div class="row">
                                    ...
                                    <select id="moderator" name="moderator" class="form-control form-control-sm" required>
                                        @foreach ($project->assignments as $assignment)
                                            @if ($assignment->employee->status == 0)
                                                <option class="text-danger" value="{{ $assignment->user_id }}"
                                                    {{ old('moderator', $moderator->id) == $assignment->user_id ? 'selected' : '' }}>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select id="status" name="status" class="form-control form-control-sm" required>
                                        <option value="Dev"
                                            {{ old('status', $project->status) == 'Dev' ? 'selected' : '' }}>Dev</option>
                                        <option value="Live"
                                            {{ old('status', $project->status) == 'Live' ? 'selected' : '' }}>Live</option>
                                    </select>
                                    @error('status')
                                        <span class="error-message text-danger">{{ $message }}</span>
                                    @enderror
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input id="doi" name="date" type="date" class="form-control"
                                        value="{{ old('date',optional($project->date)->format('Y-m-d')) }}">
                                    @error('date')
                                        <span class="error-message text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
