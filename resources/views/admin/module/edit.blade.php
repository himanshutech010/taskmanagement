            ...
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="forms-sample" method="POST" action="{{ route('admin.modules.update', $module->id) }}"
                            ...
                            <div class="row">
                                    ...
                                    <select id="project" name="project_id" class="form-control form-control-sm" required
                                        ...
                                        @foreach ($projects as $project)
                                            <option value="{{ $project->id }}"
                                                {{ $module->project_id == $project->id ? 'selected' : '' }}>
                                                {{ $project->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input id="module_name" class="form-control" type="text" name="name"
                                        value="{{ old('name', $module->name) }}" placeholder="Enter Module Name" required>
                                    @error('name')
                                        <span class="error-message text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-6">
                                    <label for="employee-checkboxes">Assigned Employee(s)<span
                                            class="text-danger">*</span></label>
                                    <div id="employee-checkboxes" class="checkit">
                                        @foreach ($assignedEmployees as $assignment)
                                            @if ($assignment->employee->status == 0)
                                                <div class="form-check">
                                                    <input type="checkbox" class="form-check-input checkitto"
                                                        id="employee-{{ $assignment->employee->id }}" name="employees[]"
                                                        value="{{ $assignment->id }}"
                                                        {{ in_array($assignment->id, $moduleAssignedEmployees) ? 'checked' : '' }}>
                                                    <label class="form-check-label text-danger"
                                                        for="employee-{{ $assignment->employee->id }}">
                                ...
                                <div class="form-group col-md-3">
                                    ...
                                    <input id="start_date" name="start_date" class="form-control" type="date"
                                        value="{{ old('start_date', $module->start_date) }}" required>
                                    @error('start_date')
                                        <span class="error-message text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group col-md-3">
                                    ...
                                    <input id="end_date" name="end_date" class="form-control" type="date"
                                        value="{{ old('end_date', $module->end_date) }}" required>
                                    @error('end_date')
                                        <span class="error-message text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                ...
                if (response.ok) {
                    data.employees.forEach(employee => {
                        ...
                        const assignedEmployees =
                            {{ json_encode($module->details->pluck('assign_project.user_id')->toArray()) }};
